...
if "!PIP_ACTION!"=="SKIP" (
    ...
    echo [INFO] Installing lxml via apt (precompiled)...
    for /f "tokens=*" %%a in ('!SSH_FULL! "LC_ALL=C apt-get install -y python3-lxml ^> /dev/null 2^>^&1 || true" 2^>nul') do rem
    echo [INFO] Removing lxml and crawl4ai from remote requirements.txt (lxml conflict)...
    for /f "tokens=*" %%a in ('!SSH_FULL! "sed -i '/^lxml/d' '%REMOTE_PATH%'/requirements.txt ^&^& sed -i '/^crawl4ai/d' '%REMOTE_PATH%'/requirements.txt" 2^>nul') do rem
    echo [INFO] Installing packages (excluding lxml and crawl4ai)...
    for /f "tokens=*" %%a in ('!SSH_FULL! "cd '%REMOTE_PATH%' ^&^& LC_ALL=C python3 -m pip install -r requirements.txt -i https://mirrors.aliyun.com/pypi/simple/ --no-cache-dir --break-system-packages" 2^>nul') do rem
    echo [INFO] Installing crawl4ai with --no-deps (lxml conflict workaround)...
    for /f "tokens=*" %%a in ('!SSH_FULL! "cd '%REMOTE_PATH%' ^&^& LC_ALL=C python3 -m pip install crawl4ai --no-deps -i https://mirrors.aliyun.com/pypi/simple/ --no-cache-dir --break-system-packages" 2^>nul') do rem
