...
class _NewsListPageState extends ConsumerState<NewsListPage> with SingleTickerProviderStateMixin {
  ...
  Widget _buildContent(AppConfig config) {
    ...
    if (newsState.isLoading && newsState.currentNews.isEmpty) return _buildLoading(config);
    if (newsState.errorMessage != null && newsState.currentNews.isEmpty) return _buildError(newsState.errorMessage!, config);
    if (newsState.currentNews.isEmpty) return _buildEmpty(newsState, config);
  ...
  Widget _buildLoading(AppConfig config) {
    final theme = config.theme;
  ...
  Widget _buildError(String msg, AppConfig config) {
    final theme = config.theme;
    final texts = config.texts;
    return Center(
      child: Column(
  ...
  Widget _buildEmpty(NewsListState newsState, AppConfig config) {
    final theme = config.theme;
    final texts = config.texts;
