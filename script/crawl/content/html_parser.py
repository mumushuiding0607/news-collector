...
使用示例：
  # 方式1：直接提取
  from crawl.content.html_parser import extract_by_source
  ...
  # 方式2：预加载正则表
  from crawl.content.html_parser import load_content_extract_patterns
  patterns = load_content_extract_patterns()
  for name, pat in patterns.items():
      print(f"  {name}: {pat[:60]}...")

  # 方式3：尝试多个数据源
  from crawl.content.html_parser import extract_by_sources
...
# sources.json 路径（相对于 script/crawl/content/ 目录）
SOURCES_PATH = Path(__file__).resolve().parent.parent.parent / "config" / "sources.json"
