...
# 提示词文件路径
PROMPT_FILE = PROMPT_DIR / "评分回测.md"
...
def call_llm(prompt: str) -> dict | None:
    ...
    try:
        import asyncio
        from llm import call_async_raw as _raw_fn

        async def _call():
            # 6000：回测输出含 有效评论数组 + 优化建议数组（每条 100-200 字），
            # 50 条评论输入 → 峰值 ~3000 tokens，留 2× 余量
...
def main():
    from script.db.backtest_db import run_backtest
    ...
    if args.news_id is not None:
        run_backtest(news_id=args.news_id)
    else:
        results = run_backtest(days=args.days)
