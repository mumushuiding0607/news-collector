...
def _run_list_discovery(url: str, list_html: str, headline: str, force_relearn: bool = False) -> tuple[dict | None, str | None, str | None]:
    ...
    # already_cleaned=True：HTML 已经过 clean_html + truncate，传入已清洗截断的 HTML
    discovered = discover_list_config(url, list_html, use_raw_fallback=True, headline=headline, already_cleaned=True, force_relearn=force_relearn)
    if not discovered:
        return None, None, "学习失败"
