...
def _save_headers_to_cache(headers: dict):
    ...
    try:
        ...
        write_json(data, CACHE_FILE, indent=0)
    except Exception:
        pass
...
def _get_condition(question: str, secondary_intent: str, headers: dict) -> dict | None:
    ...
    try:
        ...
        print(f"  [HTTP错误] _get_condition 失败: {e}, status={e.response.status_code if e.response else 'N/A'}")
        return None
    except json.JSONDecodeError as e:
        print(f"  [JSON解析错误] _get_condition 失败: {e}")
        return None
    except Exception as e:
        print(f"  [未知错误] _get_condition 失败: {type(e).__name__}: {e}")
        return None
...
def _fetch_page(url_params: dict, page: int, perpage: int, headers: dict, question: str) -> list | None:
    ...
    try:
        ...
        print(f"  [HTTP错误] _fetch_page 失败: {e}, status={e.response.status_code if e.response else 'N/A'}")
        return None
    except json.JSONDecodeError as e:
        print(f"  [JSON解析错误] _fetch_page 失败: {e}")
        return None
    except Exception as e:
        print(f"  [未知错误] _fetch_page 失败: {type(e).__name__}: {e}")
        return None
...
def _query_wencai_simple(question: str, secondary_intent: str, page: int, perpage: int, loop: int, headers: dict) -> dict:
    ...
    for i in range(loop):
        ...
        try:
            ...
            # 如果是401Unauthorized，自动刷新token重试一次
            if response.status_code == 401:
                # 刷新token后重试
                fresh_headers = get_headers(force_refresh=True)
                request_headers = {
                    **fresh_headers,
                    "Origin": "https://www.iwencai.com",
                    "Referer": f"https://www.iwencai.com/unifiedwap/result?w={encoded_question}",
...
def _simplify_data(all_datas: list, secondary_intent: str) -> list:
    ...
    for item in all_datas:
        if secondary_intent == "zhishu":
            ...
            price = _get_field(item, "指数@收盘价") or _get_field(item, "指数@最新价") or ""
